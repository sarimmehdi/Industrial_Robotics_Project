...
class Robot:
	def __init__(self):
		...
		self.farthestObsInFront = np.array([0.0, 0.0])
		self.visitedPts = []
		self.obsDir = 'None'
	...
	#theta is angle os rotation of robot frame wrt map frame
	...
	def inRangeOfPrevVisitedPts(self, candidate):
		for pt in self.visitedPts:
			if np.linalg.norm(pt-candidate) <= 2.0:
				return True
		return False
	...
	def laserScannerCallback(self, laserScanner):
		...
		for i in range(270, 360):
			...
			if laser_range <= 0.5 and min_range <= laser_range <= max_range:
				#obsX = self.curPos[0] + laser_range*cos(radians(i))
				#obsY = self.curPos[1] + laser_range*sin(radians(i))
		...
		for i in range(320, 360):
			...
			if laser_range <= minVal and (min_range <= laser_range <= max_range):
				...
				self.obsDir = 'left'
			if laser_range >= maxVal and (min_range <= laser_range <= max_range):
				obsCoords = self.getCoordsInRefFrame(np.array([laser_range*cos(radians(i)), laser_range*sin(radians(i))]))
				if not self.inRangeOfPrevVisitedPts(obsCoords):
					maxVal = laser_range
					self.farthestObsInFront[0] = obsCoords[0]
					self.farthestObsInFront[1] = obsCoords[1]
			if laser_range <= 0.3:
				left_and_right += 1
	...
	def checkIfPointBehind(self, twistMsg, Kh):
		...
		if abs(degrees(angleDiff)) > 90:
			rospy.loginfo("ROBOT NOT FACING GOAL!")
